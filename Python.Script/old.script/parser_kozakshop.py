...
PAGES_START = 1
PAGES_COUNT = 1
OUT_FILENAME = 'kozakshop'
OUT_XLSX_FILENAME = 'kozakshop'
VENDOR = 'kozakshop.com.ua'
HOST = 'https://kozakshop.com.ua'
...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://kozakshop.com.ua/velosipedy/filter/bikeSize={rost}/'
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            available = soup.find('div', class_='product-header__availability')
            ...
            #sku
            sku = soup.find('div', class_='product-header__code').get_text(strip=True)

            #brand
            brand = soup.find('div', class_='product__section product__section--header').find('meta',itemprop='name').get('content')

            #category
            category = ''

            for num, c in enumerate(soup.find_all('div', class_='breadcrumbs-i'),start=0):
                category = c.find('span',itemprop='name').get_text(strip=True)
                if category == name:
                    category = soup.find_all('div', class_='breadcrumbs-i')[num-1].find('span',itemprop='name').get_text(strip=True)
            ...
            # options
            techs = {}
            for row in soup.find_all('tr', class_='product-features__row'):
                nameoption = row.find('th', class_='product-features__cell product-features__cell--h')
                if nameoption is None:
                    continue
                nameoption = nameoption.get_text(strip=True)

                valueoption = row.find('td', class_='product-features__cell')
            ...
            price = soup.find('div', class_='product-price__item').find('meta',itemprop='price').get('content')
