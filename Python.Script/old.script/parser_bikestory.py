...
OUT_FILENAME = 'bikestory'
OUT_XLSX_FILENAME = 'bikestory'

VENDOR = 'bikestory.com.ua'
HOST = 'https://bikestory.com.ua/'
...
def crawl_products(pages_count):
    ...
    fmt = 'https://bikestory.com.ua/velosipedy/page/{page}/'
...
def parse_products(urls, withloadimage):
    ...
    #urls = ['https://bikestory.com.ua/https://bikestory.com.ua/velosiped-28-merida-mission-cx-600-light-green-black-2020/']
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            #brand
            brand = soup.find('div',class_='brand-meta').find('a').get_text(strip=True)
            ...
            if js is None:
                available = 'Продано'
                producttitlesingl = soup.find('div', class_='pc-product-single__summary')

                av = producttitlesingl.find('div', class_='stock in-stock')
                if av is None:
                    continue
                ...
                sku = producttitlesingl.find('span', class_='sku_wrapper').find('span').get_text(strip=True)

                if len(sku) == 0:
                    continue
                pp = producttitlesingl.find('div', class_='product-price product-price--lg')

                if not pp.find('del') is None:
                    op = pp.find('del').find('span', class_='woocommerce-Price-amount amount')
                    p = pp.find_all('span', class_='woocommerce-Price-amount amount')[1]
                else:
                    p = pp.find('span', class_='woocommerce-Price-amount amount')
                    op = p
                ...
                for num, param in enumerate(d, start=1):
                    ...
                    oldprice = price_without_space(oldprice)
                    price = price_without_space(price)
                    if param['attributes'].__contains__('attribute_pa_razmer-ramy'):
