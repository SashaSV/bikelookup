...
OUT_FILENAME = 'stylus'
OUT_XLSX_FILENAME = 'stylus'

VENDOR = 'stylus.ua'
HOST = 'https://stylus.ua'
...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://stylus.ua/velosipedy/p{page}/'
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        i = 0
        for tag in soup.find_all('div', class_='product-inner-block'):

            available = tag.find('span',class_='not-available')
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            # name
            name = soup.find('h1', class_='page-name').get_text(strip=True)
            ...
            techs = {}
            table = ''
            for tab in soup.find_all('div', class_='tabs-wrap'):
                if tab.get('id') == 'tab-tech-spec':
                    table = tab
                    break
            for row in table.find_all('tr', class_='item'):
                ...
                if nameoption == 'Тип дорожного велосипеда':
            ...
            js = soup.find_all('script', type='application/ld+json')[1].contents[0]
            d = json.loads(js)

            sku = d['mpn']

            price = soup.find('div', class_='regular-price').contents[0]
            oldprice = price
