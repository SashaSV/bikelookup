...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://y.ua/velosipedy-11413/c11413/?p={page}'
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        i = 0
        for tag in soup.find_all('div', class_='inner-block'):

            available = tag.find('div', class_='not-available')
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            # name
            name = soup.find('h1', class_='title-block').get_text(strip=True)
            ...
            # options
            techs = {}
            for tab in soup.find_all('div', class_='content-item'):
            ...
            js = soup.find_all('script', type='application/ld+json')[0].contents[0]
            d = json.loads(js)

            sku = d['mpn']

            price = str(d['offers']['price'])
            oldprice = price
