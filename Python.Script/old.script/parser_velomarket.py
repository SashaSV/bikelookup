...
PAGES_START = 1
PAGES_COUNT = 21
OUT_FILENAME = 'velomarket'
OUT_XLSX_FILENAME = 'velomarket'
VENDOR = 'velomarket.org.ua'
HOST = 'https://velomarket.org.ua'
...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://velomarket.org.ua/category/velosipedy/?page={page}'
    ...
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        for tag in soup.find_all('div', class_='product-layout product-grid col-lg-3 col-md-4 col-sm-6 col-xs-6'):

            available = tag.find('input').get('btn btn-default s-empty-sku')
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            # options
            techs = {}
            table = soup.find('table', class_='s-features-wrapper')
            for row in table.find_all('tr'):
                nameoption = row.find('td', class_='name').get_text(strip=True)
                valueoption = row.find('td', class_='value').get_text(strip=True)

                if nameoption == 'Тип велосипеда':
                    category = valueoption
                    continue

                if nameoption == 'Производитель':
                    brand = valueoption
                    continue
            ...
            available = 'Продано'
            av = soup.find('strong', class_='stock-high')
            ...
            paramsku = soup.find_all('div', class_='skus-item')
            for param in paramsku:

                inputparam = param.find('input')

                if inputparam is None:
                    continue

                sku = inputparam.get('data-articul')

                if sku == '':
                    sku = inputparam.get('data-kod')

                if sku == '':
                    continue

                size = param.find('span', itemprop='name').get_text(strip=True)
                ...
                price = inputparam.get('data-price')

                price = price.replace('грн', '')
                price = price_without_space(price)

                oldprice = inputparam.get('data-compare-price')
