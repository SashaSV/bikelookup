...
PAGES_START = 1
PAGES_COUNT = 27
OUT_FILENAME = 'veloplaneta'
OUT_XLSX_FILENAME = 'veloplaneta'
VENDOR = 'veloplaneta.ua'
HOST = 'https://veloplaneta.ua'
...
def crawl_products(pages_count):
    ...
    fmt = 'https://veloplaneta.ua/velosipedy/status-is-v-nalichii-or-skoro-v-magazine?pagen={page}'
    ...
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        for tag in soup.find_all('div', class_='_8y'):
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        soup = get_soup(url+'/characteristics')
        if soup is None:
            break

        try:
            #name
            name = soup.find('h1', class_='_1p').get_text(strip=True)
            ...
            available = soup.find('div', class_='_rp')

            if available is None:
                available = 'Скоро в магазине'
            else:
                available = available.get_text(strip=True)
                if available == 'Забрать в магазине':
                    available = 'В наличии'
            ...
            for row in soup.find_all('div', class_='_pt'):
                nameoption = row.find('div', class_='_pu').get_text(strip=True)
                valueoption = row.find('div', class_='_pv').get_text(strip=True)

                if nameoption == 'Размер рамы':
                    size = valueoption
                    continue

                if nameoption == 'Категория велосипеда':
            ...
            sku = soup.find('span', class_='_o5').get_text(strip=True)

            baseprice = soup.find('div', class_='_ri')
            price = oldprice = '0'

            if not baseprice is None:
                oldprice = baseprice.get_text(strip=True)

            p = soup.find('div', class_='_rn')
            ...
            #print(data)
            appintodata(data, url, name, sku, brand, category, price, oldprice, VENDOR, size, available, techs, images)
