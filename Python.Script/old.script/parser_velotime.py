...
PAGES_START = 1
PAGES_COUNT = 26
OUT_FILENAME = 'velotime'
OUT_XLSX_FILENAME = 'velotime'
VENDOR = 'velotime.com.ua'
HOST = 'https://velotime.com.ua'
...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://velotime.com.ua/ua/category/velo/?page={page}&sort=total_sales&order=DESC'
    ...
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        if soup is None:
            break
        i = 0
        for tag in soup.find_all('div', class_='product'):

            available = tag.find('span').get('price price-new nowrap price-new-custom')
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            # options
            techs = {}
            table = soup.find('table', class_='table table-hover features')
            for row in table.find_all('tr'):
                nameoption = row.find('td', class_='name')
                if nameoption is None:
                    continue
                nameoption = nameoption.get_text(strip=True)

                valueoption = row.find('td', class_='value')
            ...
            paramsku = soup.find('form', id='cart-form').find_all('div', itemprop='offers')
            for param in paramsku:
                ...
                sku = inputparam.get('value')

                size = param.find('meta', itemprop='name').get('content')
                ...
                price = param.find('meta', itemprop='price').get('content')
