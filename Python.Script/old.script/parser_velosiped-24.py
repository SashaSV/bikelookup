...
PAGES_START = 1
PAGES_COUNT = 8
OUT_FILENAME = 'velosiped-24'
OUT_XLSX_FILENAME = 'velosiped-24'
VENDOR = 'velosiped-24.com.ua'
HOST = 'https://velosiped-24.com.ua'
...
def crawl_products(pages_count):
    ...
    fmt = 'https://velosiped-24.com.ua/velosipedy/?PAGEN_1={page}/'
    ...
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        for tag in soup.find_all('div', class_='item_info'):

            href = tag.find('div', class_='item-title').find('a').get('href')
            url = '{0}{1}'.format(HOST, href)
            available = tag.find('span', class_='value font_sxs').get_text(strip=True)
            if url.strip() == (HOST.strip() + '/') or (available != 'Есть в наличии'):
                continue
...
def parse_products(urls, withloadimage):
    ...
    #urls = ['https://velosiped-24.com.ua/product-graviynyy-velosiped-pride-rocx-8-1-28-oranzhevyy-2020/?oid=108584']
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            # brand
            brand = ''
            b = soup.find('div', class_='brand')

            if b != None:
                brand = b.find('meta', itemprop='brand').get('content')

            # category
            category = soup.find_all('span', class_='breadcrumbs__item-name font_xs')[2].contents[0]

            if category == '':
                continue

            # size
            s = soup.find_all('span', class_='show_class bx_item_section_name')
            if not s is None:
                size = ''
            ...
            s = soup.find('span', class_='article__value')
            if s is None:
                continue
            ...
            prices = soup.find('div', class_='prices_block')
            ...
            p = prices.find('div', class_='price_matrix_wrapper')
            if not p is None:
                pp = p.find('div', class_='price font-bold font_mxs')
                if not pp is None:
                    price = pp.get('data-value')
                else:
                    price = \
                    prices.find('div', class_='js_price_wrapper').find('div', class_='price font-bold font_mxs').find(
                        'span', class_='values_wrapper').contents[0]

                price = price.replace('грн.', '')
                price = price_without_space(price)

                op = p.find('div', class_='price discount')
                if not op is None:
                    oldprice = op.get('data-value')

                if oldprice is None:
                    oldprice = prices.find('div', class_='js_price_wrapper').find('div', class_='price discount').find(
                        'span', class_='values_wrapper').contents[0]
            ...
            available = 'Продано'
            av = prices.find('span', class_='value font_sxs')
            if not av is None:
                available = av.contents[0]

            if price.isdigit() and int(price) > 0 and available == 'Есть в наличии':
                available = 'В наличии'

            appintodata(data, url, name, sku, brand, category, price, oldprice, VENDOR, size, available, techs,
                            images)

            # baseprice = soup.find('div', class_='old')

            # if not baseprice is None:
            #    oldprice = baseprice.get_text(strip=True)

            # p = soup.find('span', class_='price')
            # if not p is None and len(p.contents) > 0 :
            #    price = p.contents[0]

            # av = soup.find('div', class_='_kr')
            # if av is None:
            # else:
