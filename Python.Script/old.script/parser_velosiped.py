...
PAGES_START = 1
PAGES_COUNT = 5
OUT_FILENAME = 'velosiped'
OUT_XLSX_FILENAME = 'velosiped'
VENDOR = 'velosiped.com'
HOST = 'https://velosiped.com/'
...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://velosiped.com/tovary/velosipedi.html?display=50&bid_page={page}'
...
def parse_products(urls, withloadimage):
    ...
    #urls = ['https://velosiped.com/tovary/velosipedi/velosiped-trinx-m116-expert-elite-trinx-275-bilijchervonijblakitnij-21-10030179.html']
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            sku = soup.find('span', itemprop='mpn').get_text(strip=True)

            # size
            size = ''

            price = soup.find('i', itemprop='price')
            ...
            oldprice = soup.find('div', 'old-price')
            ...
            available = 'В наличии'
            a = soup.find('span', 'cat-deliv orange-colors')

            if not a is None:
                available = 'Доставка 1-3 дня'

            params = soup.find_all('div', class_='other-sizes')
            ...
            paramscolor = params[0].find_all('a')
            for param in paramscolor:
                href = param.get('href')
                if not href is None:
                    url = '{0}{1}'.format(HOST, href)
                    if url.strip() == (HOST.strip() + '/'):
            ...
            for param in paramsku:
                ...
                if not href is None:
                    ...
                    size = param.get_text(strip=True)
                    if len(size) > 5:
                        size = ''
