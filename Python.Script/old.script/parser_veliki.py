...
PAGES_COUNT = 53
OUT_FILENAME = 'veliki'
OUT_XLSX_FILENAME = 'veliki'
VENDOR = 'veliki.com.ua'
HOST = 'https://veliki.com.ua'
...
def crawl_products(pages_count):
    ...
    fmt = 'https://veliki.com.ua/bikes/page={page}/'
    ...
    for page_n in range(PAGES_START, PAGES_START + pages_count):
        ...
        for tag in soup.find_all('div', class_='holder kExist'):


            href = tag.find('strong', class_ = 'name').contents[0].get('href')
            url = 'https:{0}'.format(href)
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            sp_year = soup.find('div', class_='years').get_text(strip=True)
            ...
            """
            paramsku = soup.find('div', class_='params')
            script = paramsku.find('script', type = 'text/javascript').contents[0].strip()

            #sku = skuBase.find('span', class_='_75').get_text(strip=True)

            aaa = script.split('window.')
            for aa in aaa:
                if (aa.strip() != '' and aa.find('gParamsArr') >= 0 ):
                    script = '"'+aa.replace(' =', '":')
            ...
            for num, param in enumerate(d['gParamsArr'],start=1):
                sku = param['code_prov']

                price = param['price']
                oldprice = param['price2']
                ...
                if price.isdigit():
                    ...
                    # print(data)
                'см'
                sizes = param['name'].replace(' ', '/', 1).split('/')

                #if techs_new.__contains__('sp_height'):
                #if techs_new.__contains__('sp_size'):
                techs_new = techs.copy()
                size = sizes[0]
                if param.__contains__('min_size'):
                        min_size = str(int(float(param['min_size'])))
                        max_size = str(int(float(param['max_size'])))
                        techs_new['sp_size'] = '{0}-{1} см'.format(min_size, max_size)
                else:
                    techs_new['sp_size'] = sizes[1]
