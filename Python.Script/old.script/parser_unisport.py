...
PAGES_START = 1
PAGES_COUNT = 18
OUT_FILENAME = 'unisport'
OUT_XLSX_FILENAME = 'unisport'
VENDOR = 'unisport.ua'
HOST = 'https://unisport.ua'
...
def crawl_products(pages_count):
    ...
    """
    urls = []
    fmt = 'https://unisport.ua/velosipedy/filter/page={page};rostelosipedista={rost}/'

    rosts = [9, 14, 15, 17, 365, 366, 367, 368, 370, 382, 383, 416, 417, 418]

    for r in rosts:
        for page_n in range(PAGES_START, PAGES_START + pages_count):
            print('rost:{0} page: {1}'.format(r, page_n))
            ...
            for tag in soup.find_all('div', class_='catalogCard-title'):
...
def parse_products(urls, withloadimage):
    ...
    for number, url in enumerate(urls, start=1):
        ...
        try:
            ...
            # brand
            brand = ''
            b = soup.find('div', itemprop='brand')

            if b != None:
                brand = b.find('meta', itemprop='name').get('content')

            # category
            category = ''
            c = soup.find_all('div', class_='breadcrumbs-i')
            if not c is None:
                if len(c) >= 2:
                    c = c[2].find('span', itemprop='name')
                    if not c is None:
                        category=c.get_text()
            ...
            s = soup.find('div', class_='product-header__code product-header__code--filled')
            if s is None:
                continue
            ...
            sku = s.contents[2].strip()

            if len(sku) == 0:
                continue

            prices = soup.find('div', class_='product-price__box')
            ...
            p = prices.find('div', class_='product-price__item')

            if p is None:
                p = prices.find('div', class_='product-price__item product-price__item--new')

            pp = p.find('meta', itemprop='price')

            if not pp is None:
                price = pp.get('content')

            price = price.replace('грн.', '')
            price = price_without_space(price)

            op = prices.find('div', class_='product-price__old-price')
            if not op is None:
            ...
            available = 'Продано'
            av = soup.find('div', class_='product-header__availability')
            if not av is None:
